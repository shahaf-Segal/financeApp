...
export default function SearchID(props) {
  ...
  const coinRender = CurrCoin ? (
    <CoinCard
      turnOffSearch={true}
      coin={CurrCoin}
      changeFav={props.changeFav}
      isFav={props.isFavFunc(paramId)}
      loggedIn={props.loggedIn}
    />
  ) : (
    <h1 className="text-center">Search to Render</h1>
  );
      ...
      <div className="d-flex justify-content-center bg-dark py-2">
        <div className="input-group rounded w-50">
          <input
            ...
            placeholder="search for coins by name"
            aria-label="Search"
